...
public class RedisKit<T> {
    ...
    public RedisKit(RedisTemplate<String, T> redisTemplate) {
     ...
     * @param dbIndex
     */
    public RedisKit selectDbIndex(int dbIndex) {
     ...
     * @return true 成功 false失败
     */
    public boolean hmset(String key, String item, Object value, long seconds) {
        try {
            redisTemplate.opsForHash().put(key, item, value);
            if (seconds > 0) {
                expire(key, seconds);
     ...
     * @return 成功个数
     */
    public long sSet(String key, long seconds, T... values) {
        try {
     ...
     * @return
     */
    public boolean lSet(String key, T value, long seconds) {
        try {
            redisTemplate.opsForList().rightPush(key, value);
            if (seconds > 0) expire(key, seconds);
            return true;
     ...
     * @return
     */
    public boolean lSet(String key, List<T> value, long seconds) {
        try {
            redisTemplate.opsForList().rightPushAll(key, value);
            if (seconds > 0) expire(key, seconds);
            return true;
